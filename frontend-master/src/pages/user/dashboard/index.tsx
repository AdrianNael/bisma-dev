...
const Dashboard = ({ id: userId, role: _role }: Props) => {
    ...
    const fetchTotal = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_ENDPOINT}/api/dashboard/summary/${userId}`,
          {
            credentials: "include",
          },
        );
  ...
  return (
    <Layout title={pageTitle}>
      <div className="p-2 sm:p-4">
        ...
        {/* Expenses + WaitingPayment: stack on mobile, side-by-side on md+ */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="md:col-span-3">
            <Expenses userId={userId ?? undefined} />
          </div>
        </div>

        {/* OnGoingProject + TopMagang: stack on mobile, side-by-side on md+ */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <OnGoingProject userId={userId ?? undefined} />
          <TopMagang userId={userId ?? undefined} />
        </div>
      </div>
    </Layout>
  );
