...
const StudentManagement = (_props: Props) => {
  ...
  const fetchStudentData = async () => {
    ...
    try {
      ...
      const response = await axiosJWT.get(`${APIEndpoint}/api/mahasiswa/get`, {
        params: params,
      });
  ...
  return (
    <Layout title="Manage Users - Students">
      <div className="flex justify-center m-4 mt-6">
        <div className="flex flex-col gap-6 w-full">
          <div className="flex justify-between gap-4">
            <Link
              href={"/admin/manageuser/student/new"}
                  ...
                  <tbody>
                    {studentData.map((student) => (
                      <tr key={student.id} className="border-b border-gray-200">
                        <td className="py-3 px-6 text-black">{student.id}</td>
                        <td className="py-3 px-6 text-black">{student.nama}</td>
...
export default withPage(StudentManagement);
