...
const AddStudentUser = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosJWT.post(
        `${APIEndpoint}/api/mahasiswa/create`,
        formData,
        {
          headers: {
      ...
      if (response.status === 201) {
        toast.success("Student user added successfully!");
        router.push("/admin/manageuser/student");
      } else {
        toast.error(response.data.message || "Failed to add student user.");
      }
    } catch (error: any) {
  ...
  return (
    <Layout title="Tambah User - Mahasiswa">
      <div className="w-full min-h-[calc(100vh-2rem)] p-6">
        <h2 className="text-2xl font-bold mb-6">Tambah User - Mahasiswa</h2>
        <form
        ...
        >
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="flex flex-col">
              <label className="mb-2 font-semibold">Departemen</label>
              <select
...
export default withPage(AddStudentUser);
