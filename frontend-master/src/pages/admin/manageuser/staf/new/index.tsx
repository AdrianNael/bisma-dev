...
const AddStaffUser = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        toast.error(response.data.message || "Failed to add staff user.");
      }
    } catch (error: any) {
  ...
  return (
    <Layout title="Tambah User - Staff">
      <div className="w-full min-h-[calc(100vh-2rem)] p-6">
        <h2 className="text-2xl font-bold mb-6">Tambah User - Staff</h2>
        <form
...
export default withPage(AddStaffUser);
