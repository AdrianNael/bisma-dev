...
const NewKategori = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const categoryResponse = await axiosJWT.post(
      ...
      if (categoryResponse.status === 200) {
        ...
        // Step 2: Create incentive with the new category ID
        const incentiveResponse = await axiosJWT.post(
          `${APIEndpoint}/api/incentive`,
          {
            id_kategori: newCategoryId,
        ...
        if (incentiveResponse.status === 200) {
          ...
          toast.error(
            incentiveResponse.data.message || "Gagal menambahkan insentif.",
          );
        }
      } else {
        toast.error(
          categoryResponse.data.message || "Gagal menambahkan kategori.",
        );
      }
    } catch (error: any) {
...
export default withPage(NewKategori);
