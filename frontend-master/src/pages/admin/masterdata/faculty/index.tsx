...
const FacultyManagement = (_props: Props) => {
  ...
  const fetchFacultyData = async () => {
    ...
    try {
      ...
      setTotalPages(response.data.paging?.total_page || 1);
      setIsFirstLoad(1);
    } catch (error) {
  ...
  const confirmDeletion = async (facultyId: number) => {
  ...
  const handleDelete = async (facultyId: number) => {
    try {
      const response = await axiosJWT.delete(
        `${APIEndpoint}/api/faculty/${facultyId}`,
      );
  ...
  return (
    <Layout title="Master Data - Fakultas">
      <div className="flex justify-center m-4 mt-6">
        <div className="flex flex-col gap-6 w-full">
          <div className="flex justify-between gap-4">
            <Link
              href={"/admin/masterdata/faculty/new"}
...
export default withPage(FacultyManagement);
