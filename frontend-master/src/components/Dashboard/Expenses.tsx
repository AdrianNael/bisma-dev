...
const Expenses = ({ userId, showKategoriChart = false }: ExpensesProps) => {
  ...
  useEffect(() => {
    const fetchAvailableYears = async () => {
      try {
        ...
        const url = userId
          ? `${API_ENDPOINT}/api/dashboard/available-years/${userId}`
          : `${API_ENDPOINT}/api/dashboard/global/available-years`;
    ...
    <div className="flex flex-col rounded-lg shadow-lg shadow-gray-500 w-full p-4 bg-slate-50 h-full min-h-[20rem] md:min-h-[26rem]">
      ...
      <div className="w-full flex justify-center flex-1">
        <LineChart
          userId={userId}
          year={selectedYear}
          showKategoriChart={showKategoriChart}
        />
      </div>
    </div>
  );
